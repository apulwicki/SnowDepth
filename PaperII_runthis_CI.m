...
% BMS_Circle.G4 = zeros(maxN, numRand,7);
% BMS_Circle.G2 = zeros(maxN, numRand,7);
% BMS_Circle.G13 = zeros(maxN, numRand,7);
...
% Linear regression (BMA)
    cd BMS
    [BMSinit, BMSres] = BMS_R_CI(WBinput, TOPOinput);
    cd ..
    
    for gg = 1:3;        glacier = char(options.glacier(gg));
    BMS_Circle.(glacier)(n,x,:) = BMSinit.(glacier){:,1};
    end

 end
end
 
save('BMS_Circle.mat','BMS_Circle')