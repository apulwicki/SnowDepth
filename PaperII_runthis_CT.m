...
% BMS_CentreTransect.G4 = zeros(maxN, numRand,7);
% BMS_CentreTransect.G2 = zeros(maxN, numRand,7);
% BMS_CentreTransect.G13 = zeros(maxN, numRand,7);
...
% Linear regression (BMA)
    cd BMS
    [BMSinit, BMSres] = BMS_R_CT(WBinput, TOPOinput);
    cd ..
    
    for gg = 1:3;        glacier = char(options.glacier(gg));
    BMS_CentreTransect.(glacier)(n,x,:) = BMSinit.(glacier){:,1};
    end

 end
end
 
save('BMS_CentreTransect.mat','BMS_CentreTransect')