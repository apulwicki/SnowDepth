...
% BMS_HourCircle.G4 = zeros(maxN, numRand,7);
...
% Linear regression (BMA)
    cd BMS
    [BMSinit, BMSres] = BMS_R_HC(WBinput, TOPOinput);
    cd ..
    
    for gg = 1:3;        glacier = char(options.glacier(gg));
    BMS_HourCircle.(glacier)(n,x,:) = BMSinit.(glacier){:,1};
    end

 end
end
 
save('BMS_HourCircle.mat','BMS_HourCircle')
