...
% %         synMLR(ss).(namesP{p})(mc).(glacier)   = coeffs;
        ...
        cd BMS
        [BMSinit, BMSres] = BMS_R_HC(WBinputN, TOPOinput.(namesP{p}));
        cd ..

        for gg = 1:3;        glacier = char(options.glacier(gg));
            coeffs = BMSinit.(glacier){:,1};
        BMS_HC.(glacier)(ss,gg,:) = coeffs;
...
%         SynPred(ss).(namesP{p})(mc).(glacier) = swe_pred;
        ...
        synRMSE_HC.(namesP{p}).(glacier)(ss,mc) = sqrt(nanmean(nanmean((pred_dist-true_dist).^2)));
        end
...
save('PaperII_AA_BMS_HC.mat', 'synRMSE_HC', 'BMS_HC')