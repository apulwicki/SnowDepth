...
load TopoSWE.mat 
run OPTIONS.m
load PaperII_AblationArea_SFUruns.mat BMS
...
if ~isempty(BMS(r).(pattern_name))
    
    for g = 1:3;    glacier = options.glacier{g};
        sweBMS      = repmat(BMS_alldata.(glacier){5,1}, options.mapsize(g,:));
        mlrCoeff    = BMS(r).(pattern_name).(glacier){1:4,1};    topoCoeff = fieldnames(topo_full.G4);
            %multiply coeffs and add them
        for m = 1:length(mlrCoeff)
            param     = topoCoeff{m};
            sweT      = topo_full.(glacier).(param)*mlrCoeff(m);
