...
Bw_obs = [0.6, 0.3208, 0.2516]; % ablation area only
% Bw_obs = [0.6, 0.52, 0.39]; % whole glacier
...
% DETERMINISTIC
    ...
    clear syn_measure_tmp syn_measure real_measure_tmp
        ...
        det_rmseLR.(namesP{p}).(glacier)(ss,mc,sampleN) = sqrt(mean((syn_measure_tmp-real_measure_tmp).^2));
...
%         predLR(ss).(namesP{p})(mc).(glacier) = swe_pred;
        ...
        rmseLR.(namesP{p}).(glacier)(ss,mc,sampleN) = sqrt(mean((syn_measure_tmp-real_measure_tmp).^2));
    end
