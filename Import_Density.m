...
if options.TubeDensity == 1
    A = cell2mat(Density.tube(:,2:19));
    
    M =  [Density.tube(:,1), num2cell(nanmean(A,2)),...
                             num2cell(nanstd(A,1,2)),...
                             num2cell(min(A,[],2)),...
                             num2cell(max(A,[],2)),...
                             num2cell(sum(~isnan(A),2))];

    Density.tube = [M, Density.tube(:,20:23)];
    Density.pitANDtube(:,2:6) = M(index_SPSWE,2:6);
    Density.zigzagtube(:,2) = M(index_zigzag,2);

elseif options.TubeDensity == 2
