...
% %         synMLR(ss).(namesP{p})(mc).(glacier)   = coeffs;
        ...
        cd BMS
        [BMSinit, BMSres] = BMS_R_CE(WBinputN, TOPOinput.(namesP{p}));
        cd ..

        for gg = 1:3;        glacier = char(options.glacier(gg));
            coeffs = BMSinit.(glacier){:,1};
        BMS_CE.(glacier)(ss,gg,:) = coeffs;
...
%         SynPred(ss).(namesP{p})(mc).(glacier) = swe_pred;
        ...
        synRMSE_CE.(namesP{p}).(glacier)(ss,mc) = sqrt(nanmean(nanmean((pred_dist-true_dist).^2)));
        end
...
save('PaperII_AA_BMS_CE.mat', 'synRMSE_CE', 'BMS_CE')