...
name = [name(1:index-1);str2double(strcat(num2str(name(index)),'000'));name(index:end)]; %The name of the imaginary WP is 'OldWP#' and then a 0
...
for j = 1:length(easting)-1 %for each WP
    ...
    if name(j+1)<1000 %ensures that two subsequent points are from the same shape
        ...
        if name(j)>370 && name(j)<529 || name(j) == 3710 || name(j) == 5190 %WPs when only two probers
            ...
            closest(:,end+1) = [line(:,index);(name(j+1)+0.1)]; %Enters the easting, northing, and GPS WP into variable 'closest'
            [~, index] = min(abs(distinterp-val(1,2)));
            closest(:,end+1) = [line(:,index);(name(j+1)+0.2)];
        else %
            val = [10 20 30]; %Chosen distance from GPS of probers
            [~, index] = min(abs(distinterp-val(1,1)));
            closest(:,end+1) = [line(:,index);(name(j+1)+0.1)];
            [~, index] = min(abs(distinterp-val(1,2)));
            closest(:,end+1) = [line(:,index);(name(j+1)+0.2)];
            [~, index] = min(abs(distinterp-val(1,3)));
            closest(:,end+1) = [line(:,index);(name(j+1)+0.3)];
        end
    end
