...
for num_model = 1:size(realCoeffs.(namesP{p}).G4,2)
    
for g = 1:3;    glacier = options.glacier{g};
coeffs.(glacier) = [realCoeffs.(namesP{p}).(glacier)(ind,num_model,1), realCoeffs.(namesP{p}).(glacier)(ind,num_model,2),...
                    realCoeffs.(namesP{p}).(glacier)(ind,num_model,3), realCoeffs.(namesP{p}).(glacier)(ind,num_model,4),...
                    realCoeffs.(namesP{p}).(glacier)(ind,num_model,5)];
end

real_measure = SampledCell(sweBMS_alldata);
